...
require_once($_SERVER['DOCUMENT_ROOT'].'/api'.'/models/response/image_handler_response.php');
require_once($_SERVER['DOCUMENT_ROOT'].'/api'.'/services/log/log_service.php');
...
class Image_handler {
  ...
  */
  ...
  @image_handler_Response
  */
  function to_base64($path) : Image_handler_response {
    $path = __DIR__.$path;
    $error_message = "";
    $response = new Image_handler_response();

    $data = file_get_contents($path);

    if(!$data) {
      $error_message = "Could not find image with given path. \n\n Path: ".$path;
      //log_service::new(1,"to_base64",$error_message);
  ...
  */
  function save($base64Image) : Image_handler_response {
    $response = new Image_handler_response();
    ...
    if($this->output_folder) {
      ...
      if (!is_dir($path_to_folder)) {
        ...
        if(!$create_dir) {
          ...
          log_service::new(1,"Save Image", $error_msg);
      ...
      if($image) {
        ...
        if(!$save) {
          log_service::new(1,"Save Image","Failed to save Image. Filepath: " . $short_file_path);
