...
class ApnsPHP_Log_Embedded implements ApnsPHP_Log_Interface
{
	...
	{
		$response = sprintf("%s ApnsPHP[%d]: %s\n",
			date('r'), getmypid(), trim($sMessage)
		);

		// require_once($root = realpath($_SERVER['DOCUMENT_ROOT']) . "/api/app/v2/" . 'config/dbconfig.php');
		// $conn = get_connection();

		// mysqli_query($conn, "INSERT INTO ios_push_notifications_log (message) VALUES ('$response')");
	}
}
